...
export default function (mbMap) {
  ...
  return {
    ...
    setup: (actionCb, features, theme) => {
      ...
      eventLoad = () => {
        mbMap
          .on('draw.create', eventCreate)
          .on('draw.selectionchange', eventSelection)
          .on('draw.update', eventUpdate);
      };

      mbMap.on('load', eventLoad);
    ...
    /**
     * Removes the mapbox drawing control from the map.
     */
    destroy: () => {
      if (!mbDraw) return;

      mbMap
        .off('draw.create', eventCreate)
        .off('draw.selectionchange', eventSelection)
        .off('draw.update', eventUpdate)
        .off('load', eventLoad);

      mbMap.removeControl(mbDraw);
      mbDraw = null;
    }
  };
}
